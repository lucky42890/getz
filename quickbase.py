...
#                attrib = dict((k, str(v)) for k, v in attrib.iteritems())
                attrib = dict( (k, str(v)) for k, v in iter(attrib.items()))
            else:
                attrib = {}
            sub_element = etree.SubElement(request, field, **attrib)
#            if not isinstance(value, basestring):
            if not isinstance(value, str):    #basestring no longuer exist in Py3
                value = str(value)
            sub_element.text = value

#        print ("0 --> ",request_fields )                                        #Debug

#        for field, values in request_fields.iteritems():
        for field, values in request_fields.items():   #iteritems is gone in Py 3
...
#                print ("Values: ",values )                                      #debug
    ...
    def request(self, action, database, request, required=None, ticket=True,
        ...
        response = ""
#        print ("8--> data: ", data)                                            #debug
#        print ("8--> url: ", url)                                              #debug
#        print ("8--> headers: ", headers)                                      #debug
        request = requests.post(url, data, headers=headers, stream=True)
#        print ("7--> request: ", request)                                        #debug
        if request.status_code == 200:
            for chunk in request.iter_content(2048):
                encode = chardet.detect(chunk)['encoding']
                if encode != 'utf-8':
#                    print ("1--> chunk: ", chunk)                               #debug
#                    chunk = chunk.decode(encode, 'replace').encode('utf-8')
#                    chunk = chunk.decode("utf-8", "replace")                    #updated decode tp Py3
                    chunk = chunk.decode("utf-8", "ignore")                    #updated decode tp Py3
#                    print ("2--> chunk: ", chunk)                               #debug
                response += chunk
#                print ("3--> chunk: ", chunk)                                   #debug
...
#            parsed = etree.fromstring(response)
            parsed = etree.fromstring(response.encode('utf-8'), parser=utf8_parser)                                      #debug
